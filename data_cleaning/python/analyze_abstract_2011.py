...
def get_keywords(key_str_list):
	keywords = []
	for i in range(7, len(key_str_list) - 1):
...
def gen_row(key_str_list):
	...
	# Authors / authors
	row.append(get_authors(key_str_list[4]))

	# Institute / institute
	row.append(get_institute(key_str_list[4]))

	# Abstract / abstract
	row.append(filter_tags(key_str_list[5]))

	# Disclosures / disclosures
	row.append(get_disclosures(key_str_list[6]))
...
def get_key_str_list(url):
	...
	key_str_list = [elt[:elt.find('</td>')].strip() for elt in key_str_list]
	if key_str_list[0] == 'This presenter will not attend':
		key_str_list = key_str_list[1:]
...
def main(input_file, output_file):
	with open(output_file, 'wb') as f:
	    writer = csv.writer(f)
	    writer.writerow(['pres_title', 'location', 'pres_time', \
	    	'authors', 'institute', 'abstract', 'disclosures', \
	    	'keywords'])
